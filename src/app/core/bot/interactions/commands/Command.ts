...
export class Command {
	...
	constructor(
		...
		if (option?.isPrivateGuildCommand) {
			this.isPrivateGuildCommand = option.isPrivateGuildCommand;
		}
		if (!option?.subcommandOnly) {
			if (execute) {
				...
				throw new ApplicationFatalError({
					message: `Unexpected missing mandatory execute fonction in command ${name}`,
				});
			}
		}
	}
	...
	private buildCommand() {
		...
		for (const subcommand of subcommands) {
			...
			if (subcommandInfo) {
				command.addSubcommand(subcommandInfo.slashCommandSubcommandBuilder);
			}
		}
	...
	private async executeCommand(interaction: ChatInputCommandInteraction) {
		...
		if (subcommandName) {
			...
			throw new ApplicationFatalError({
				message: `Unexpected missing mandatory execute fonction in command ${this.name}. Error code: ${code}`,
			});
		}
	}
	...
	private getSubcommands() {
		...
		try {
			subcommandsFiles.push(
				...readdirSync(path.join(this.commandPath, 'subcommands')),
			);
		} catch (error) {
			return subcommandsFiles;
		}
		return subcommandsFiles.filter(
			(file) =>
				file.endsWith('.js') &&
				file.startsWith(
					this.functionality[0].toUpperCase() + this.functionality.slice(1),
				),
		);
	}
	...
	private loadSubcommand(subcommand: string) {
		...
		const subcommandInfo = require(subcommandPath)
			.subcommandInfo as ISubcommand;
		this.subcommands.set(
			subcommandInfo.slashCommandSubcommandBuilder.name,
			subcommandInfo,
		);
